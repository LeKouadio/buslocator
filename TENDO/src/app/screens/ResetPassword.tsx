...
import { Lock, ArrowLeft, Bus, MapPin, ShieldCheck } from 'lucide-react';
...
export const ResetPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token) {
      toast.error(t('auth.error_invalid_token'));
      return;
    }
    if (!validate()) return;
    ...
    try {
      ...
      const message = err.response?.data?.message || t('auth.error_invalid_token');
      toast.error(message);
    } finally {
      setLoading(false);
    }
  ...
  if (!token) {
    return (
      <div className="size-full flex items-center justify-center p-6 text-center">
        <div>
          <ShieldCheck className="w-16 h-16 text-red-500 mx-auto mb-4" />
          <h2 className="text-xl font-bold mb-2">{t('auth.error_invalid_token')}</h2>
          <Button onClick={() => navigate('/login')}>{t('welcome.login')}</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="size-full bg-background flex flex-col overflow-hidden relative transition-colors duration-300">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="text-[#FFE0B2] text-[14px] font-medium mt-1">
